...
describe('MaxSubArray', () => {
  it('Finds correct max sum for [1, -3, 2, 1, -1]', () => {
    const arr = [1, -3, 2, 1, -1];
    assert.strictEqual(MaxSubArray.findMaxSubArray(arr), 3);
  });

  it('Finds correct max sum for single element arr [1]', () => {
    const arr = [1];
    assert.strictEqual(MaxSubArray.findMaxSubArray(arr), 1);
  });
  ...
  it('Finds correct max sum for all positive arr = [1, 2, 3, 4, 5]', () => {
    const arr = [1, 2, 3, 4, 5];
    assert.strictEqual(MaxSubArray.findMaxSubArray(arr), 15);
  });

  it('Returns null for an empty arr = []', () => {
    const arr = [];
    assert.isNull(MaxSubArray.findMaxSubArray(arr));
  });
